...
# Common Linux/bash runner for K=8,D=2048 improvement experiments.
# This ports the K=24 run2/run5 parameter sets without changing their
# network/refiner hyperparameters. Training uses random_ellipses val500 as the
# primary validation set and observes a random 100-sample Shepp-Logan subset as
...
Options:
  ...
  --output-tag TAG               Output tag. Default: a8_d2048_n{Noise}_admm80_{Experiment}
  --offline-data-dir PATH        Directory containing KD offline .pt files. Default: run_detector_KD/8_angle_det2048/offline_data
...
ALPHA_JSON="${PROJECT_ROOT}/data/alpha8_tv/alpha_selected8_dopt_hard_gap_16_30.json"
...
if [[ -z "$OUTPUT_TAG" ]]; then
    OUTPUT_TAG="a8_d2048_${NOISE_TAG}_admm80_${EXPERIMENT}"
fi
...
if [[ -z "$RESOLVED_OFFLINE_DATA_DIR" ]]; then
    RESOLVED_OFFLINE_DATA_DIR="${KD_ROOT}/8_angle_det2048/offline_data"
fi

FILE_SUFFIX="alpha8_noise${OFFLINE_NOISE_TAG}_det2048_edgewsubset"
...
set_run_env "ALPHA_CONDITION_TOP_K_OVERRIDE" "8"
...
set_run_env "NUM_ANGLES_TOTAL_OVERRIDE" "8"
set_run_env "CNN_ANGLE_INDICES_OVERRIDE" "$CNN_ANGLE_INDICES"
set_run_env "CNN_NUM_ANGLES_OVERRIDE" "8"
...
set_run_env "NUM_DETECTOR_SAMPLES_OVERRIDE" "2048"
...
echo "[$MODE] K=8 detector_samples=2048 measurements=$((K * DETECTOR_SAMPLES))"
