...
class LearnedGradientDescent(nn.Module):
    def __init__(self, beta=THEORETICAL_CONFIG["beta_vector"], height=IMAGE_SIZE, width=IMAGE_SIZE,
        ...
        self.triangular_residual_enabled = bool(
            TIME_DOMAIN_CONFIG.get("triangular_residual_channel_enabled", False)
        )
        self.triangular_residual_mode = str(
            TIME_DOMAIN_CONFIG.get("triangular_residual_mode", "damped")
        ).strip().lower()
        self.triangular_residual_damping = float(
            TIME_DOMAIN_CONFIG.get("triangular_residual_damping", 1.0e-2)
        )
        self.triangular_residual_detach = bool(
            TIME_DOMAIN_CONFIG.get("triangular_residual_detach", True)
        )
        self.triangular_residual_normalize = bool(
            TIME_DOMAIN_CONFIG.get("triangular_residual_normalize", True)
        )
        self.triangular_residual_channels = (
            self.raw_cnn_num_angles if self.triangular_residual_enabled else 0
        )

        # Optional learnable per-angle gate for triangular residual channels.
        # This is useful because the last four angles in the current 8-angle set
        # are substantially worse conditioned than the first four.
        self.triangular_angle_attention_enabled = bool(
            TIME_DOMAIN_CONFIG.get("triangular_angle_attention_enabled", False)
        )
        self.triangular_angle_logits = None
        if self.triangular_angle_attention_enabled:
            init_gates = TIME_DOMAIN_CONFIG.get(
                "triangular_angle_attention_init",
                [0.70, 0.70, 0.70, 0.70, 0.35, 0.35, 0.35, 0.35],
            )
            if len(init_gates) != self.raw_cnn_num_angles:
                init_gates = [0.5] * self.raw_cnn_num_angles
            gate_tensor = torch.tensor(init_gates, dtype=torch.float32).clamp(
                1.0e-4, 1.0 - 1.0e-4
            )
            self.triangular_angle_logits = nn.Parameter(
                torch.log(gate_tensor / (1.0 - gate_tensor))
            )

        # Optional explicit update using the averaged triangular residual correction:
        #   e_tri ~= c_true - c_current
        #   c_next = c_current + alpha * mean_i(e_tri_i) - learned_update
        self.triangular_explicit_update_enabled = bool(
            TIME_DOMAIN_CONFIG.get("triangular_explicit_update_enabled", False)
        )
        self.triangular_explicit_update_max = float(
            TIME_DOMAIN_CONFIG.get("triangular_explicit_update_max", 0.25)
        )
        tri_alpha_init = max(
            float(TIME_DOMAIN_CONFIG.get("triangular_explicit_update_alpha_init", 0.05)),
            1.0e-8,
        )
        self.triangular_alpha_raw = nn.Parameter(
    ...
    def _compose_cnn_input(
        ...
        if self.triangular_residual_enabled:
            if triangular_corr_pa is None:
                raise ValueError(
                    "triangular_residual_channel_enabled=True, but triangular_corr_pa is None."
                )
            tri_channels = torch.index_select(
                triangular_corr_pa,
                dim=1,
                index=index_tensor,
            ).squeeze(2)
    ...
    def current_triangular_alpha(self):
        alpha = F.softplus(self.triangular_alpha_raw)
        if self.triangular_explicit_update_max > 0:
    ...
    def _apply_triangular_angle_attention(self, triangular_corr_pa):
        if triangular_corr_pa is None:
            return None
        if (
            not self.triangular_angle_attention_enabled
            or self.triangular_angle_logits is None
        ):
            return triangular_corr_pa
        gate = torch.sigmoid(self.triangular_angle_logits).view(1, -1, 1, 1, 1)
        gate = gate.to(dtype=triangular_corr_pa.dtype, device=triangular_corr_pa.device)
        return triangular_corr_pa * gate

    def _triangular_explicit_update(self, triangular_corr_pa, reference):
        if (
            not self.triangular_explicit_update_enabled
            or triangular_corr_pa is None
        ):
            return torch.zeros_like(reference)
        index_tensor = torch.as_tensor(
            self.cnn_channel_indices,
            device=triangular_corr_pa.device,
            dtype=torch.long,
        )
        tri_selected = torch.index_select(
            triangular_corr_pa,
            dim=1,
            index=index_tensor,
        )
        tri_update = tri_selected.mean(dim=1)  # (B,1,H,W)
        return self.current_triangular_alpha() * tri_update
    ...
    def forward(self, coeff_initial, g_observed):
        ...
        for _ in range(self.n_iter):
            ...
            if self.triangular_residual_enabled:
                ...
                if not hasattr(op, "triangular_residual_correction_per_angle"):
                    raise ValueError(
                        "triangular_residual_channel_enabled=True, but the active operator "
                        "does not implement triangular_residual_correction_per_angle()."
                    )
                triangular_corr_pa = op.triangular_residual_correction_per_angle(
                    coeff_current,
                    g_observed_learned,
                    mode=self.triangular_residual_mode,
                    damping=self.triangular_residual_damping,
                    normalize=self.triangular_residual_normalize,
                )
            ...
            tri_update = self._triangular_explicit_update(
                triangular_corr_pa,
...
def initialize_model():
    ...
    print(
        "Triangular residual: enabled=%s mode=%s damping=%.3g channels=%d explicit_update=%s alpha=%.4f angle_attention=%s"
        % (
            bool(getattr(model.optimizer, "triangular_residual_enabled", False)),
            str(getattr(model.optimizer, "triangular_residual_mode", "disabled")),
            float(getattr(model.optimizer, "triangular_residual_damping", 0.0)),
            int(getattr(model.optimizer, "triangular_residual_channels", 0) or 0),
            bool(getattr(model.optimizer, "triangular_explicit_update_enabled", False)),
