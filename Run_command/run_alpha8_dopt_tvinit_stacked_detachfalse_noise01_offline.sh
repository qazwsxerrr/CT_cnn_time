...
# Linux/server offline-training command for:
#   selected 8 alpha angles from D-opt JSON
#   pretrained/offline data generated with TV ADMM initialization
#   learned optimizer regularization-gradient channel: dirichlet
#   data fidelity gradient channel: selected stacked A_S, 1 channel
#   physics residual correction channel: selected stacked A_S, 1 channel
#   detach_physical_grads=False
#
# Expected offline tensor files contain only:
#   coeff_true, g_observed, coeff_initial
#
# Generate examples:
#   python models/data_genoration/offline_tvinit_data.py \
#     --num-samples 8000 --data-source random_ellipses --batch-size 4 \
#     --output models/data_genoration/train8000_tvinit_alpha8_noise01.pt
#   python models/data_genoration/offline_tvinit_data.py \
#     --num-samples 500 --data-source shepp_logan --batch-size 4 --seed-offset 1000000 \
#     --output models/data_genoration/val500_shepp_tvinit_alpha8_noise01.pt
...
# Kept identical to the online TV-init run so the model metadata/config match.
# Offline train/val batches are loaded from .pt and do not recompute init online.
...
export DATA_FIDELITY_CHANNEL_MODE_OVERRIDE="stacked_selected"
...
export PHYSICS_EXPLICIT_UPDATE_ENABLED_OVERRIDE="${PHYSICS_EXPLICIT_UPDATE_ENABLED_OVERRIDE:-1}"
export PHYSICS_EXPLICIT_UPDATE_ALPHA_INIT_OVERRIDE="${PHYSICS_EXPLICIT_UPDATE_ALPHA_INIT_OVERRIDE:-0.1}"
export PHYSICS_EXPLICIT_UPDATE_MAX_OVERRIDE="${PHYSICS_EXPLICIT_UPDATE_MAX_OVERRIDE:-0.25}"

export N_ITER_OVERRIDE="8"

export INTERMEDIATE_SUPERVISION_ENABLED_OVERRIDE="1"
export INTERMEDIATE_SUPERVISION_WEIGHT_START_OVERRIDE="0.2"
export INTERMEDIATE_SUPERVISION_WEIGHT_END_OVERRIDE="1.0"
...
export OUTPUT_TAG_OVERRIDE="${OUTPUT_TAG_OVERRIDE:-alpha8_dopt_tvinit_stacked_noise01_offline_explicit_update}"
...
echo "[run] data_fidelity_channel=${DATA_FIDELITY_CHANNEL_MODE_OVERRIDE} physics_residual=${PHYSICS_RESIDUAL_MODE_OVERRIDE} explicit_update=${PHYSICS_EXPLICIT_UPDATE_ENABLED_OVERRIDE}"
